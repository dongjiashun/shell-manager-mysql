...
# jacson
# step 1 get date by zabbix.hosts .for getting hostname
# step 2 use hostname show mysql contain ip and port to check slave 
# step 3 choice wheter you want to chioce to login in database
...
for i in ${!slaves[@]}; do
   ...
   if echo "${slaves[$i]}" | grep -E '^m.*'; then
       	#echo "$(($i+1)). ${slaves[$i]}"
	echo -e "\033[32m$(($i+1)). ${slaves[$i]}\033[0m"
   else
	echo -e "\033[31m$(($i+1)). ${slaves[$i]}\033[0m"
   	#echo "$(($i+1)). ${slaves[$i]}"
   fi
done
...
mysql -h $ip -P $port -u $user -p$pwd -A
